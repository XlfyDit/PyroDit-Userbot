...
def git():
    ...
    try:
        repo = Repo()
        LOGGER("PYRODIT").info(f"Git Client Found")
    except GitCommandError:
        LOGGER("PYRODIT").info(f"Invalid Git Command")
        ...
        LOGGER("PYRODIT").info("Fetched Latest Updates")
    ...
    if is_heroku:
        if HEROKU_API_KEY and HEROKU_APP_NAME:
            try:
                ...
                LOGGER("PYRODIT").info(f"Heroku App Configured")
...
async def create_botlog(client):
    ...
    desc = "Group Log untuk PyroDit-Userbot.\n\nHARAP JANGAN KELUAR DARI GROUP INI.\n\n✨ Powered By ~ @RapalelWak ✨"
    try:
